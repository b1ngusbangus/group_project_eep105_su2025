...
if selected == "Scatterplots":
    ...
    with sc3:
        ...
        st.caption("We see here that the datapoints are scattered but still shows a generally positive direction. Our t-value that we calculated is 2.73, meaning statistical significance at the 5% level. Therefore, we are able to reject the null hypothesis that temperature and CO2 emissions aren't correlated, which makes sense since CO2 emissions have largely shown to contribute to rising temperatures. Given that Vietnam is becoming more developed and is producing more CO2 emissions, we would expect to see it's average temperatures rising in the coming years.")
